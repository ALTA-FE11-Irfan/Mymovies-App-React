    ...
    <div className="h-[75vh] w-full pt-20">
      <div className="relative h-full w-full overflow-hidden">
        <div className="top left absolute flex h-full w-full justify-between">
          ...
          >
            <svg xmlns="http://www.w3.org/2000/svg" className="-ml-5 h-12 w-20" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        ...
        <div className="carousel w-full h-full" ref={carousel}>
          {datas.map((data, index) => (
            <div id={index.toString()} key={index} className="carousel-item relative w-full h-full">
              {content(data)}
            </div>
          ))}
        </div>
